  ...
  <div className="min-h-screen bg-gray-100 flex flex-col justify-center items-center">
    <div className="space-y-8 p-6 bg-white rounded-md shadow-lg w-3/4">
      <div key={font.name} className="space-y-4">
        <h1 className={`${font.className} text-4xl`}>{font.displayName} Sample Text</h1>
        <p className={`${font.className} text-lg`}>This is a paragraph in {font.displayName}.</p>
        <p className={`${font.className} font-bold text-lg`}>Bold text in {font.displayName}.</p>
    ...
    fonts.map((font: FontConfig) => {
      describe(`${font.displayName} Font`, () => {
        it('has the correct class for regular text', () => {
          ...
          const regularText = getByText(`This is a paragraph in ${font.displayName}.`)
